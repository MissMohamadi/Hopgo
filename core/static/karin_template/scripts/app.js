...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!loginForm) {
    console.warn("فرم لاگین پیدا نشد");
    return;
  }

  const inputField = loginForm.querySelector("input[name='username']");
  const errorMessage = loginForm.querySelector("p.text-error");
  const submitButton = loginForm.querySelector(".submit-btn");

  if (!inputField) {
    console.warn("فیلد username پیدا نشد");
    return;
  }

  if (!submitButton) {
    console.warn("دکمه ورود پیدا نشد");
    return;
  }
  ...
  function validateInput(value) {
    const normalizedValue = normalizeNationalId(value);
    ...
    if (!isValidIranianNationalId(normalizedValue)) {
  ...
  inputField.addEventListener("input", function () {
    const error = validateInput(inputField.value);

    console.log("مقدار فعلی:", inputField.value);
    console.log("خطای اعتبارسنجی:", error);

    updateUI(error);
  });

  // بررسی اولیه اگر فیلد از قبل مقدار داشت
  updateUI(validateInput(inputField.value));
});
